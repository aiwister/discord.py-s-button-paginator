...
class Page:
  def __init__(self,embed,timeout,prefix="",suffix="",author=False):
    self.embed=embed
    self.prefix=prefix
    self.suffix=suffix
    self.author=author
  ...
  async def start(self,interaction):
      if isinstance(interaction,discord.Interaction):
          msg=await interaction.response.send_message(embed=discord.Embed(description=f"{self.prefix}{self.embed[0]}{self.suffix}"),view=PageButton(emb=self.embed,author=self.author,prefix=self.prefix,suffix=self.suffix,timeout=self.timeout))
